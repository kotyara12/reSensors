...
class rIAQItem: public rSensorItem {
  public:
  ...
  private:
    ...
    float               _limit_bad = BME68x_DEFAULT_GAS_LIMIT_BAD;
    float               _limit_good = BME68x_DEFAULT_GAS_LIMIT_GOOD;
    limits_autoshift_t  _limits_autoshift = LIM_AUTOSHIFT_OFF;
    paramsEntryHandle_t _prm_hum_ratio = nullptr;
    paramsEntryHandle_t _prm_limit_bad = nullptr;
    paramsEntryHandle_t _prm_limit_good = nullptr;
    paramsEntryHandle_t _prm_autoshift = nullptr;
    
    void checkLimits(float newValue);
    value_t getHumidityScore(value_t humValue);
    value_t getGasScore(value_t gasValue);
    value_t calculateIAQ(value_t humValue, value_t gasValue);
};
